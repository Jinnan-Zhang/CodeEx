...
*  @compile:  g++ -o MyIntegraltor.cc.o MyIntegraltor.cc -fopenmp
*/
#include <iostream>
#include <cmath>
#include <omp.h>
...
//* @param fError: integration error
//* @param Tolerance: absolute tolerance and relative tolerance
//* @param maxpts: maximum number of function evaluation requested
//* @param fSize: max size of working array (explode with dimension)
template <class T>
double DoAdapIntgral(T *Obj, double (T::*p_FCN)(const double *),
                     double *xmin, double *xmax,
                     unsigned int ndim, int &status, double &RelError, double &fError,
                     double Tolerance = 1e-6, unsigned int maxpts = 100000, unsigned int fSize = 100000)
{
    // References:
    //
    ...
    if (n < 2 || n > 15)
    {
        std::cerr << "Error in AdaptiveIntegratorMultiDim::Integral: Wrong function dimension:" << n << std::endl;
        return 0;
    }
    ...
    maxpts = std::max(maxpts, irlcls); // specified maximal number of function evaluations
...
L20:
    ...
    sum1 = (Obj->*p_FCN)((const double *)z); // EvalPar(z,fParams); //evaluate function
...
L160:                                // to divide or not
    ...
    if (status == 3)
    {
        ...
        if (idvax0 < 1)
        {
            ...
            idvax0 = 1;
            std::cerr << "Error in AdaptiveIntegratorMultiDim::DoIntegral(), Logic error: idvax0 < 1!" << std::endl;
        }
...
//* @param maxpts: maximum number of function evaluation requested
//* @param fSize: max size of working array (explode with dimension)
double DoAdapIntgral(double (*p_FCN)(const double *),
                     double *xmin, double *xmax,
                     unsigned int ndim, int &status, double &RelError, double &fError,
                     double Tolerance = 1e-6, unsigned int maxpts = 100000, unsigned int fSize = 100000)
{
...
L20:
    ...
    sum1 = (p_FCN)((const double *)z); // EvalPar(z,fParams); //evaluate function
...
#pragma omp parallel for
    for (int i = 0; i < 1000; i++)
    {
        ...
        double xmax[] = {1, 1, 1};
        int status;
        double RelErr, IntErr;
        double result = DoAdapIntgral(SimFCN, xmin, xmax, 3,
                                      status, RelErr, IntErr);
        printf("i:%d\t status: %d\t Relative Err: %.8f\t Integration Err: %.8f\t Result: %.8f\n", i, status, RelErr, IntErr, result);
    }
    return 0;
}

//a simple function fo integral
double SimFCN(const double *x)
{
    return (3 * x[0] * x[0] + x[1] + x[2]);
}
