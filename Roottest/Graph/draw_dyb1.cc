...
//      day.push_back(1.3506e+09+a*50*86400);
        ...
        if(c>d) theta8AD.push_back(c);
        else theta8AD.push_back(d);
a++; 
     }
     ...
     while(inputFile1.getline(buffer1, sizeof(buffer1))){
        ...
        if(c>d) theta8AD_stat.push_back(c);
        else theta8AD_stat.push_back(d);
     }
...
//cout<<b<<" "<<c<<" "<<d<<endl;
     ...
     for(int i=0; i<DYBshapeDaydelta.size(); i++) { DYBshapeSendelta->SetPoint(i,DYBshapeDaydelta[i],DYBshapedelta[i]); DYBshapeSen_statdelta->SetPoint(i,DYBshapeDaydelta[i],DYBshape_statdelta[i]); }
   //
